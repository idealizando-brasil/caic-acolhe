...
export default function Dashboard(){
 ...
 const [email,setEmail]=useState(""); const [role,setRole]=useState(""); const [school,setSchool]=useState<School|null>(null); const [studentsCount,setStudentsCount]=useState(0); const [activeCount,setActiveCount]=useState(0); const [todayCount,setTodayCount]=useState(0); const [urgentCount,setUrgentCount]=useState(0); const [pendingReports,setPendingReports]=useState(0); const [referrals,setReferrals]=useState<Referral[]>([]); const [appointments,setAppointments]=useState<Appointment[]>([]); const [teamNotifications,setTeamNotifications]=useState(0);
 const load=useCallback(async()=>{
  ...
  const raw=membership?.schools as unknown; const current=(Array.isArray(raw)?raw[0]:raw) as School|undefined; if(!current)return; setSchool(current); setRole(membership?.role||"");
  const start=new Date();start.setHours(0,0,0,0);const end=new Date(start);end.setDate(end.getDate()+1);
  const [studentResult,referralResult,urgentResult,appointmentResult,requestResult,notificationResult]=await Promise.all([
   ...
   supabase.from("referral_notifications").select("id",{count:"exact",head:true}).eq("school_id",current.id).eq("user_id",user.id).is("acknowledged_at",null)
  ]);
  ...
  const reportRequests=(requestResult.data||[]) as unknown as ReportRequest[];setPendingReports(reportRequests.filter(item=>!item.teacher_reports?.[0]?.submitted_at).length);setTeamNotifications(notificationResult.count||0);
 },[router,supabase]);
 useEffect(()=>{load()},[load]);
 const isMultiprofessional=role==="psychologist"||role==="social_worker";
 const notice=useMemo(()=>isMultiprofessional&&teamNotifications>0?{title:`${teamNotifications} ${teamNotifications===1?"novo encaminhamento":"novos encaminhamentos"} para a Equipe Multiprofissional`,text:"Abra as demandas e confirme sua ciência.",href:"/dashboard/encaminhamentos",action:"Ver demandas"}:studentsCount===0?{title:"Turmas configuradas — aguardando os alunos",text:"Aguarde a importação dos estudantes pela gestão.",href:"/dashboard/alunos",action:"Ver alunos e turmas"}:{title:"Ambiente institucional em funcionamento",text:`${studentsCount} estudantes cadastrados e disponíveis para acompanhamento.`,href:"/dashboard/encaminhamentos",action:isMultiprofessional?"Ver demandas":"Novo encaminhamento"},[isMultiprofessional,studentsCount,teamNotifications]);
 if(!school)return <main className="loading">Carregando ambiente seguro…</main>;
 return <AppShell email={email}><header><div><p className="eyebrow green">{school.name.toUpperCase()} · 2026</p><h1>Olá, equipe do CAIC</h1><p className="muted">Aqui está o panorama atualizado do acolhimento escolar.</p></div><Link href="/dashboard/encaminhamentos" className="notification-button" aria-label="Ver notificações da equipe"><Bell/>{(isMultiprofessional?teamNotifications:urgentCount)>0&&<span>{isMultiprofessional?teamNotifications:urgentCount}</span>}</Link></header>
 ...
 <section className="dashboard-grid"><article className="card dashboard-panel"><div className="section-title"><div><h2>{isMultiprofessional?"Demandas da Equipe Multiprofissional":"Fila de acolhimento"}</h2><p className="muted">{isMultiprofessional?"Psicologia e Serviço Social":"Encaminhamentos recentes e prioridades"}</p></div><Link className="button-link" href="/dashboard/encaminhamentos">Ver fila</Link></div>{!referrals.length?<div className="empty"><ClipboardList/><h3>Nenhum encaminhamento ainda</h3><p>Os novos casos aparecerão aqui para triagem.</p></div>:<div className="dashboard-referrals">{referrals.map(item=><Link href="/dashboard/encaminhamentos" key={item.id}><span className={`dashboard-priority ${item.priority}`}>{priorityLabels[item.priority]}</span><div><b>{item.students?.full_name||"Aluno"}</b><small>{item.students?.classes?.name||"Sem turma"} · {statusLabels[item.status]||item.status}</small><p>{item.reason_summary}</p></div><ArrowUpRight/></Link>)}</div>}</article>
