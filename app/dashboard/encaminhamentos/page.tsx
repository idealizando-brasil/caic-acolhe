...
import { AlertTriangle, Bell, CheckCircle2, ClipboardList, Pencil, Plus, Search, Trash2, UserCheck, X } from "lucide-react";
...
type Referral = {
  ...
  status: Status; school_actions: string | null; due_date: string | null; next_action: string | null; next_action_due: string | null; destination: string; created_at: string;
  students: { full_name: string; classes: { name: string } | null } | null;
  profiles: { full_name: string | null; email: string } | null;
};
type ReferralNotification = { id: string; referral_id: string; acknowledged_at: string | null };
...
const emptyForm = { student_id: "", assigned_to: "", reason_summary: "", priority: "routine" as Priority, status: "received" as Status, school_actions: "", due_date: "", next_action: "", next_action_due: "" };
...
export default function Encaminhamentos() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: studentData }, { data: referralData }, { data: notificationData }] = await Promise.all([
      supabase.from("students").select("id,full_name,classes(name)").eq("school_id", current.id).eq("active", true).order("full_name"),
      supabase.from("referrals").select("id,student_id,assigned_to,reason_summary,priority,status,school_actions,due_date,next_action,next_action_due,destination,created_at,students(full_name,classes(name)),profiles!referrals_assigned_to_fkey(full_name,email)").eq("school_id", current.id).order("created_at", { ascending: false }),
      supabase.from("referral_notifications").select("id,referral_id,acknowledged_at").eq("school_id", current.id).eq("user_id", user.id)
    ]);
    setStudents((studentData || []) as unknown as Student[]);
    setReferrals((referralData || []) as unknown as Referral[]);
    setNotifications((notificationData || []) as ReferralNotification[]);
  }, [router, supabase]);
  ...
  const canCreateOrDelete = role === "director" || role === "coordinator";
  const notificationByReferral = useMemo(() => new Map(notifications.map(item => [item.referral_id, item])), [notifications]);
  const pendingNotificationCount = notifications.filter(item => !item.acknowledged_at).length;
  ...
  function editReferral(item: Referral) {
    setForm({ student_id: item.student_id, assigned_to: item.assigned_to || "", reason_summary: item.reason_summary, priority: item.priority, status: item.status, school_actions: item.school_actions || "", due_date: item.due_date || "", next_action: item.next_action || "", next_action_due: item.next_action_due || "" });
  ...
  async function saveReferral(event: FormEvent) {
    ...
    const values = { school_id: school.id, student_id: form.student_id, assigned_to: form.assigned_to || null, reason_summary: form.reason_summary.trim(), priority: form.priority, status: form.status, school_actions: form.school_actions.trim() || null, due_date: form.due_date || null, next_action: form.next_action.trim() || null, next_action_due: form.next_action_due || null, destination: "multiprofessional_team", updated_at: new Date().toISOString() };
  ...
  async function acknowledgeReferral(item: Referral) {
    const notification = notificationByReferral.get(item.id);
    if (!notification || notification.acknowledged_at) return;
    setBusy(true); setMessage("");
    const { error } = await supabase.from("referral_notifications").update({ read_at: new Date().toISOString(), acknowledged_at: new Date().toISOString() }).eq("id", notification.id);
    setMessage(error ? "Não foi possível confirmar a ciência." : "Ciência do encaminhamento confirmada.");
    if (!error) await load(); setBusy(false);
  }

  async function claimReferral(item: Referral) {
    if (!school || !isMultiprofessional || item.assigned_to) return;
    setBusy(true); setMessage("");
    const { error } = await supabase.from("referrals").update({ assigned_to: userId, status: item.status === "received" ? "triage" : item.status, updated_at: new Date().toISOString() }).eq("id", item.id).eq("school_id", school.id);
    setMessage(error ? "Não foi possível assumir a referência." : "Você agora é o profissional de referência.");
    if (!error) await load(); setBusy(false);
  }

  async function removeReferral(item: Referral) {
    if (!school || !canCreateOrDelete || !confirm(`Excluir o encaminhamento de ${item.students?.full_name || "este aluno"}?`)) return;
    setBusy(true); setMessage("");
  ...
  return <AppShell email={email}>
    <header className="referrals-header"><div><p className="eyebrow green">EQUIPE MULTIPROFISSIONAL</p><h1>Encaminhamentos</h1><p className="muted">Demandas compartilhadas entre Psicologia e Serviço Social.</p></div>{canCreateOrDelete && <button className="primary-button" disabled={!students.length} onClick={() => { closeForm(); setShowForm(true); }}><Plus /> Novo encaminhamento</button>}</header>
    {isMultiprofessional && pendingNotificationCount > 0 && <div className="team-alert"><Bell /><div><b>{pendingNotificationCount} {pendingNotificationCount === 1 ? "novo encaminhamento" : "novos encaminhamentos"} para a Equipe Multiprofissional</b><p>Abra cada demanda e confirme a ciência.</p></div></div>}
    ...
    {showForm && <form className="card referral-form" onSubmit={saveReferral}>
      <div className="form-heading"><div><h2>{editingId ? "Editar encaminhamento" : "Novo encaminhamento"}</h2><p className="muted">Registre apenas informações necessárias ao cuidado e acompanhamento.</p></div><button type="button" className="close-button" onClick={closeForm}><X /></button></div>
      <div className="referral-form-grid">
        ...
        <label>Prioridade<select disabled={Boolean(editingId) && !canCreateOrDelete} value={form.priority} onChange={e => setForm({ ...form, priority: e.target.value as Priority })}>{priorities.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
        ...
        <div className="team-destination"><span>Destino</span><b>Equipe Multiprofissional</b><small>Psicologia e Serviço Social recebem esta demanda.</small></div>
        <label>Prazo para retorno<input disabled={Boolean(editingId) && !canCreateOrDelete} type="date" value={form.due_date} onChange={e => setForm({ ...form, due_date: e.target.value })} /></label>
        <label className="full-field">Motivo do encaminhamento<textarea disabled={Boolean(editingId) && !canCreateOrDelete} value={form.reason_summary} onChange={e => setForm({ ...form, reason_summary: e.target.value })} rows={4} required placeholder="Descreva objetivamente a situação observada e a necessidade de acompanhamento." /></label>
        <label className="full-field">Ações já realizadas pela escola<textarea disabled={Boolean(editingId) && !canCreateOrDelete} value={form.school_actions} onChange={e => setForm({ ...form, school_actions: e.target.value })} rows={3} placeholder="Ex.: conversa com a família, acompanhamento em sala, contato com a coordenação." /></label>
        <label className="full-field">Próxima ação<textarea value={form.next_action} onChange={e => setForm({ ...form, next_action: e.target.value })} rows={2} placeholder="O que será feito, por quem e qual o próximo passo." /></label>
        <label>Prazo da próxima ação<input type="date" value={form.next_action_due} onChange={e => setForm({ ...form, next_action_due: e.target.value })} /></label>
      </div>
    ...
    <section className="card referrals-card">
      <div className="referrals-toolbar"><div><h2>Fila de acompanhamento</h2><p className="muted">{filtered.length} {filtered.length === 1 ? "encaminhamento" : "encaminhamentos"}</p></div><div className="referral-filters"><label className="search-box"><Search /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Pesquisar aluno ou motivo" /></label><select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option value="">Todos os andamentos</option>{statuses.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></div></div>
      {!filtered.length ? <div className="empty"><ClipboardList /><h3>{referrals.length ? "Nenhum caso encontrado" : "Nenhum encaminhamento ainda"}</h3><p>{referrals.length ? "Altere a pesquisa ou o filtro." : "Os novos casos aparecerão aqui para triagem."}</p></div> : <div className="referral-list">{filtered.map(item => <article key={item.id} className={`referral-item priority-${item.priority}`}><div className="referral-main"><div className="referral-title"><span className={`priority-label ${item.priority}`}>{priorities.find(row => row[0] === item.priority)?.[1]}</span><h3>{item.students?.full_name || "Aluno"}</h3><small>{item.students?.classes?.name || "Sem turma"}</small></div><p>{item.reason_summary}</p>{item.school_actions && <div className="school-actions"><b>Ações da escola:</b> {item.school_actions}</div>}<div className="referral-meta"><span>Destino: Equipe Multiprofissional</span><span>Referência: {item.profiles?.full_name || item.profiles?.email || "a definir"}</span><span>Prazo: {item.due_date ? new Intl.DateTimeFormat("pt-BR", { timeZone: "UTC" }).format(new Date(`${item.due_date}T12:00:00Z`)) : "não definido"}</span></div>{item.next_action && <div className="next-action"><b>Próxima ação:</b> {item.next_action}{item.next_action_due ? ` · até ${new Intl.DateTimeFormat("pt-BR", { timeZone: "UTC" }).format(new Date(`${item.next_action_due}T12:00:00Z`))}` : ""}</div>}</div><div className="referral-controls"><select aria-label="Atualizar andamento" value={item.status} disabled={busy} onChange={e => updateStatus(item, e.target.value as Status)}>{statuses.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select>{isMultiprofessional && !notificationByReferral.get(item.id)?.acknowledged_at && <button className="acknowledge" disabled={busy} onClick={() => acknowledgeReferral(item)}><CheckCircle2 /> Confirmar ciência</button>}{isMultiprofessional && !item.assigned_to && <button className="claim" disabled={busy} onClick={() => claimReferral(item)}><UserCheck /> Assumir referência</button>}<div><button onClick={() => editReferral(item)} title="Atualizar caso"><Pencil /></button>{canCreateOrDelete && <button className="delete" onClick={() => removeReferral(item)} title="Excluir"><Trash2 /></button>}</div></div></article>)}</div>}
    </section>
  </AppShell>;
}
