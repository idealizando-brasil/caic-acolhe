  ...
  return <AppShell email={email}>
    ...
    {showForm && <form className="card referral-form" onSubmit={saveReferral}>
      ...
      <div className="referral-form-grid">
        ...
        <label className="full-field">Motivo do encaminhamento<textarea disabled={Boolean(editingId) && !canCreateOrDelete} value={form.reason_summary} onChange={e => setForm({ ...form, reason_summary: e.target.value })} rows={4} required placeholder="Descreva objetivamente a situação observada e a necessidade de acolhimento." /></label>
        <label className="full-field">Ações já realizadas pela escola<textarea disabled={Boolean(editingId) && !canCreateOrDelete} value={form.school_actions} onChange={e => setForm({ ...form, school_actions: e.target.value })} rows={3} placeholder="Ex.: conversa com a família, acolhimento em sala, contato com a coordenação." /></label>
